...
class ControllerPaymentBtcLightningPayment extends Controller {
	public function index() {
		...
		//$data['timezone'] = $this->config->get('btc_lightning_payment_timezone');
		$data['total_btc'] = $this->session->data['total_btc'];
		$data['total_order_usd'] = $order_info['total'];
		$data['text_pubkey_full'] = sprintf($this->language->get('text_pubkey'), $this->config->get('btc_lightning_payment_node_pubkey'));
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/btc_lightning_payment.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/btc_lightning_payment.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/btc_lightning_payment.tpl', $data);
		}
	}

	public function getPaymentRequest($memo='',$satoshi=0){
		$lnd_ip         = $this->config->get('btc_lightning_payment_node_ip');
		$lnd_port       = $this->config->get('btc_lightning_payment_node_port');
		$macaroon_base64= $this->config->get('btc_lightning_payment_invoice_macaroon_hex');
	...
	public function confirm() {
		if ($this->session->data['payment_method']['code'] == 'btc_lightning_payment') {
			$this->load->language('payment/btc_lightning_payment');
		...
		$comment .= sprintf($this->language->get('text_paymenttime'), $this->session->data['order_time'], $this->session->data['order_time']);
			...
			$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('btc_lightning_payment_order_status_id'), $comment, true);
		}
	}
}